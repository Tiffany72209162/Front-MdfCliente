...
const ModalEdit = ({visible, idCliente }) => {
  ...
  const getClienteId = async(id) => {
     await axios.get('https://apimdf.devopsacademy.pe/entidades/clientes/' + id )
    .then(response => {
  ...
  const onEdit = () => {
    form.validateFields()
        .then((values) => {        
        dispatch(updateCliente(`https://apimdf.devopsacademy.pe/entidades/clientes/${values.id}/`,values))
        form.resetFields()        
        setVisible(!Visible)
            ...
            <Col span={10}>
              <Card title="Datos de Usuario" style={{ width: 400, textAlign: 'center' }}>
                  ...
                  <Button 
                    type='secondary' 
                    size='large' 
                    shape='round' {...ubc}> Cambiar Contraseña </Button>
                </Card>
            </Col>    
          </Row>
        </div>
