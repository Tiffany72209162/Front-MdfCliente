...
const RightSidebar = () => {
	...
	const currentUserFab = useSelector((content) => content.ChatApp.currentUserFab);
	...
	const LoadChats = async (item) => {
		dispatch({ type: "SELECTED_USER", payload: { id: item.id, teamName: item.name } });
		dispatch({
			type: "LOADING_CHAT",
			payload: {
				loading: true,
		...
		try {
			...
			let emit = userid;
			let res = await Axios.post(`${Url}chat/receptor/getMessages`, { emit, receiver, pagina }, config);
			...
			if (response.success) {
				...
				dispatch({ type: "LOADING_CHAT", payload: { loading: false, estado: true } });
				...
				<Container className="themed-container p-0">
					<div className="modal-header p-l-20 p-r-20">
						<Col sm="8" className="p-0">
							<h6 className="modal-title font-weight-bold">Seleccione un usuario</h6>
						</Col>
					</div>
				</Container>
